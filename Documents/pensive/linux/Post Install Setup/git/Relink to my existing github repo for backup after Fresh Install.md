...
> This guide outlines the process for pushing to an existing github repo to continue backing-up your local files after a fresh install of arch. this is after you've already restored with git_dawn checkout -f [[Restore Backup On a Fresh Install]]  
...
> This command permanently deletes the `dawn` directory from your home folder. 
...
```bash
git_dawn commit -m "fresh install first commit to the same old git repo"
```
