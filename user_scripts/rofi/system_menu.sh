...
run_term_hold() {
	...
	validate_launch_target "$1" || return 0
	spawn "$TERMINAL" --hold --class "$class" -e "$@"
	exit 0
}
...
open_editor() {
	...
	spawn "$TERMINAL" --class "nvim_config" -e "${EDITOR_CMD[@]}" "$file"
	exit 0
}
...
show_utils_menu() {
	...
	while :; do
		...
		'  Screenshot (Swappy)')
			...
			spawn "$BASH" -lc 'region=$(slurp) || exit 0; grim -g "$region" - | swappy -f -'
			exit 0
			;;
		'󰅇  Clipboard Persistence')
...
validate_startup() {
	require_commands rofi fd file realpath xdg-open "$TERMINAL" "${EDITOR_CMD[0]}" || exit 1
