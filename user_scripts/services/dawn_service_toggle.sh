...
# TARGET:       Artix Linux / Hyprland / OpenRC
# ENGINE:       Dusky TUI Engine v3.9.1 (Adapted)
# VERSION:      4.0.0
# ==============================================================================
#
# v4.0.0 CHANGELOG:
#   - PORT: Full rewrite from systemd to OpenRC (rc-service/rc-update/rc-status).
#   - FEATURE: Tab-based UI with "User Services" and "System Services" tabs.
#   - FEATURE: User services use --user flag (no sudo needed).
#   - FEATURE: System services require sudo credential prompting.
#   - FEATURE: Dynamic service discovery — services not found are hidden.
...
# ── Service Definitions ──
# SYNTAX: "openrc_service_name|Friendly Description"
# Services that are not installed (init script not found) are silently skipped.
# User services: rc-service --user / rc-update --user (no sudo)
# System services: rc-service / rc-update (require sudo)

declare -ra USER_SERVICE_DEFS=(
	"hyprsunset|Night Light (Blue Light Filter)"
	"battery-notify|Battery Level Notifications"
	"network-meter|Waybar Network Traffic Monitor"
	"dawn-control-center|Dusky Control Center"
	"dawn-sliders|Dusky Sliders"
	"update-checker|Automatic Update Checker"
...
declare -ra SYSTEM_SERVICE_DEFS=(
	...
	"bluetooth|Bluetooth"
	"cronie|Cron Daemon"
	"elogind|Elogind Session Manager"
	"zram-generator|ZRAM Swap Compressor"
)
...
# Check if an OpenRC service is installed.
# For user services: checks rc-service --user, ~/.config/rc/init.d, /etc/user/init.d, dotfiles.
# For system services: checks rc-service -l and /etc/init.d.
# Returns 0 if installed, 1 if not.
is_unit_installed() {
	local unit="$1" scope="$2"
	local -r DOTFILES_USER_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/../openrc/user/init.d"
	local -r USER_RC_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/rc/init.d"

	if [[ "$scope" == "user" ]]; then
		# Check multiple locations for user services
		[[ -x "${USER_RC_DIR}/${unit}" ]] && return 0
		[[ -x "${DOTFILES_USER_DIR}/${unit}" ]] && return 0
		[[ -x "/etc/user/init.d/${unit}" ]] && return 0
		rc-service --user --list 2>/dev/null | grep -q "^${unit}$" && return 0
		return 1
	else
		# Check system locations
		[[ -x "/etc/init.d/${unit}" ]] && return 0
		rc-service -l 2>/dev/null | grep -q "^${unit}$" && return 0
		return 1
	fi
}

# Register services into tab arrays, skipping uninstalled ones.
# This runs once at startup — O(n) rc-service calls, not per-frame.
...
# Refresh all statuses for the current tab using OpenRC
refresh_tab_statuses() {
	...
	for ((i = 0; i < count; i++)); do
		if [[ "${_r_scope[i]}" == "user" ]]; then
			if rc-service "${_r_units[i]}" --user status 2>/dev/null | grep -qE '\[started\]'; then
				_r_status[i]="active"
			else
				_r_status[i]="inactive"
			fi
		else
			if rc-status --all 2>/dev/null | grep -qE "^\s*${_r_units[i]}\s+\[started\]"; then
				_r_status[i]="active"
			else
				_r_status[i]="inactive"
			fi
...
refresh_single_status() {
	...
	if [[ "${_rs_scope[idx]}" == "user" ]]; then
		if rc-service "${_rs_units[idx]}" --user status 2>/dev/null | grep -qE '\[started\]'; then
			_rs_status[idx]="active"
		else
			_rs_status[idx]="inactive"
		fi
	else
		if rc-status --all 2>/dev/null | grep -qE "^\s*${_rs_units[idx]}\s+\[started\]"; then
			_rs_status[idx]="active"
		else
			_rs_status[idx]="inactive"
		fi
...
toggle_selected() {
	...
	if [[ "$current_state" == "active" ]]; then
		if [[ "$scope" == "user" ]]; then
			rc-service "$unit_name" --user stop 2>/dev/null || true
			rc-update --user del "$unit_name" default 2>/dev/null || true
			if rc-service "${_t_units[SELECTED_ROW]}" --user status 2>/dev/null | grep -qE '\[started\]'; then
				result=1
			fi
		else
			sudo rc-service "$unit_name" stop 2>/dev/null || true
			sudo rc-update del "$unit_name" default 2>/dev/null || true
		fi
		if ((result == 0)); then
		...
		if [[ "$scope" == "user" ]]; then
			rc-update --user add "$unit_name" default 2>/dev/null || true
			rc-service "$unit_name" --user start 2>/dev/null || true
			if ! rc-service "${_t_units[SELECTED_ROW]}" --user status 2>/dev/null | grep -qE '\[started\]'; then
				result=1
			fi
		else
			sudo rc-update add "$unit_name" default 2>/dev/null || result=$?
			sudo rc-service "$unit_name" start 2>/dev/null || true
		fi
		if ((result == 0)); then
...
draw_ui() {
	...
	# ── Title line ──
	strip_ansi "$APP_TITLE"
	local -i t_len=${#REPLY}
	strip_ansi "$APP_VERSION"
	local -i v_len=${#REPLY}
	...
	if ((count > 0)); then
		...
		buf+=$'\n'" ${C_GREY}Service:${C_RESET} ${C_WHITE}${_det_units[SELECTED_ROW]}${C_RESET}  ${C_GREY}Scope:${C_RESET} ${scope_label}${CLR_EOL}"$'\n'
	else
		buf+=$'\n'"${CLR_EOL}"$'\n'
	fi
...
handle_key() {
	...
	r | R)
		refresh_tab_statuses
		FEEDBACK_MSG="${C_CYAN}■ Status refreshed${C_RESET}"
		FEEDBACK_COUNTDOWN=2
		;;
	' ' | '' | $'\n') toggle_selected ;;
	l | L) toggle_selected ;;
	h | H) return ;;
...
main() {
	...
	if ! command -v rc-service &>/dev/null; then
		log_err "Missing dependency: rc-service (OpenRC)"
		exit 1
	fi
