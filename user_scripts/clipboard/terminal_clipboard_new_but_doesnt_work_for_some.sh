...
# Artix Linux / Hyprland / OpenRC clipboard utility
...
# CLIPHIST_DB_PATH can be set via environment or hyprland config
# If not set, cliphist uses default cache location (~/.cache/cliphist.db)
# No UWSM dependency - uses standard environment variables
...
display_image() {
	...
	[[ -f "$img" ]] || {
		printf '\e[31mImage not found\e[0m\n'
		return 1
	}
...
main() {
	...
	--preview)
		[[ $# -ge 2 ]] || {
			log_err "--preview requires an item"
			exit 1
		}
		shift
		cmd_preview "$1"
	...
	--delete)
		[[ $# -ge 2 ]] || {
			log_err "--delete requires an item"
			exit 1
		}
		shift
		cmd_delete "$1"
