...
quiesce_snapper() {
	if rc-service snapper-timeline status >/dev/null 2>&1 || rc-service snapper-cleanup status >/dev/null 2>&1; then
		sudo rc-service snapper-timeline stop 2>/dev/null || true
		sudo rc-service snapper-cleanup stop 2>/dev/null || true
	fi
}
...
preflight_checks
quiesce_snapper
execute "Install Snapper" install_packages
post_install_checks
