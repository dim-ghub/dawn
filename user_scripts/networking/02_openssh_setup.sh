...
if [[ "$SSH_UNIT_TYPE" == "socket" ]]; then
	socket_config=$(if command -v rc-service >/dev/null 2>&1; then
		# For OpenRC, read the socket service file
...
if [[ "$SSH_UNIT" == "sshd.service" ]]; then
	if rc-update show default | grep -q sshd.socket; then
...
if rc-service sshd status >/dev/null 2>&1; then
	...
	if ! rc-service sshd start >/dev/null 2>&1; then
		...
		die "Fix the issue and re-run. Check 'rc-status' for details."
	fi
