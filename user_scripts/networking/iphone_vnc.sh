...
check_dependencies() {
	...
	if ! rc-service usbmuxd status >/dev/null 2>&1; then
		log "Starting usbmuxd..."
		run_sudo rc-service usbmuxd start || warn "Failed to start usbmuxd"
	fi
}
