...
if command -v awww >/dev/null 2>&1; then
	...
	notify-send "Favorites" "swww/awww not installed"
	exit 1
fi
...
get_current_wallpaper() {
	...
	img=$("$WALLPAPER_CMD" query 2>/dev/null | sed -n 's/.*image: //p')
...
apply_wallpaper() {
	...
	if [[ -x "$THEME_CTL" ]]; then
		"$THEME_CTL" set "$img" 2>/dev/null || "$WALLPAPER_CMD" img "$img"
	else
...
# bindd = $mainMod, H, Add Favorite, exec, $user_scripts/theme_matugen/theme_favorites_ctl.sh toggle
#
# bindd = $mainMod SHIFT, apostrophe, Cycle Favorite, exec, $user_scripts/theme_matugen/theme_favorites_ctl.sh cycle
#
