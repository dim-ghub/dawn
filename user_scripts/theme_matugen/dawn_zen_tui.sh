#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# Dusky Browser Theme Manager - Master v1.6.0
# -----------------------------------------------------------------------------
...
declare -r APP_TITLE="Dusky Browser Themer"
...
declare -ra BROWSER_PRIORITY=("zen" "zen_alt" "firefox" "librewolf")
...
ensure_matugen_integration() {
	...
	export HOOK_CMD="    ln -nfs \"$HOME/.config/matugen/generated/zen_websites.css\" \"${FF_PROFILE}/chrome/colors.css\""
    ...
    END {
        ...
        for (i=1; i<=n; i++) {
            if (lines[i] ~ /^[[:space:]]*#?[[:space:]]*\[templates\.zen_websites\]/) {
                start = i
