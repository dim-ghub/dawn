...
is_active() {
	...
	sys) rc-service "$name" status >/dev/null 2>&1 ;;
	user) pgrep -x "$name" &>/dev/null ;;
	script)
		local pids
...
perform_stop() {
	...
	script)
		...
		;;
	sys)
		if ! { ((EUID == 0)) && rc-service "$name" stop || sudo rc-service "$name" stop; } 2>/dev/null; then
			return 1
		fi
		sleep 0.2
		! is_active "$name" "sys"
		;;
	user)
		if ! pkill -x "$name" 2>/dev/null; then return 1; fi
		sleep 0.2
		! is_active "$name" "user"
		;;
	*) return 1 ;;
...
for item in "${SELECTED_ITEMS[@]}"; do
	if [[ "$item" == sys:* && $EUID -ne 0 ]]; then
		...
		! sudo -v && {
			gum style --foreground 196 "Auth failed. Aborting."
			exit 1
		}
		break
	fi
