...
register_items() {
	...
	register 2 "Critical Action" 'CMD_CRITICAL|cycle||systemctl suspend,loginctl suspend,systemctl hibernate,poweroff,loginctl lock-session||' "$(command -v loginctl >/dev/null 2>&1 && echo 'loginctl suspend' || echo 'systemctl suspend')"
}
...
main() {
	...
	command -v awk &>/dev/null || {
		log_err "Required: awk"
		exit 1
	}
	command -v grep &>/dev/null || {
		log_err "Required: grep"
		exit 1
	}
