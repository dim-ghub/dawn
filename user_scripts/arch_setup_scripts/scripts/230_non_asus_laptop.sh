...
setup_rules() {
	...
	# ASUS Control (bindd)
	add_rule "$HYPR_KEYBINDS" 'bindd = , XF86Launch3, ASUS Control, exec, $terminal --class asusctl.sh -e sudo $scripts/asus/asusctl.sh'
	...
	add_rule "$HYPR_KEYBINDS" 'bindd = $mainMod, O, TTS Kokoro GPU, exec, wl-copy "$(wl-paste -p)" && $scripts/tts_stt/kokoro_gpu/speak.sh'
	add_rule "$HYPR_KEYBINDS" 'bindd = $mainMod SHIFT, O, TTS Kokoro CPU, exec, wl-copy "$(wl-paste -p)" && $scripts/tts_stt/kokoro_cpu/kokoro.sh'
	add_rule "$HYPR_KEYBINDS" 'bindd = $mainMod SHIFT, I, STT Whisper CPU, exec, $scripts/tts_stt/faster_whisper/faster_whisper_stt.sh'
	add_rule "$HYPR_KEYBINDS" 'bindd = $mainMod, I, STT Parakeet GPU, exec, $scripts/tts_stt/parakeet/parakeet.sh'
