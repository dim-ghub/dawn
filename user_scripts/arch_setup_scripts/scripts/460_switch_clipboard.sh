...
readonly STATE_FILE="${HOME}/.config/dawn/settings/clipboard_state"
...
generate_terminal_block() {
	printf '%s\n' \
		"${MARKER_START}" \
		'unbind = $mainMod, V' \
		'bindd = $mainMod, V, Clipboard History, exec, $scripts/clipboard/close_terminal_clipboard.sh kitty --class terminal_clipboard.sh -e "$scripts/clipboard/terminal_clipboard.sh"' \
		"${MARKER_END}"
}
