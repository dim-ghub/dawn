...
force_clean_locks() {
	# No uwsm locks to clean in OpenRC mode
	return 0
}
...
main() {
	...
	# Dependencies Check
	local dep
	for dep in waybar stty sed setsid; do
	...
	[[ -d "$CONFIG_ROOT" ]] || {
		log_err "Directory ${CONFIG_ROOT} missing."
		exit 1
	}
