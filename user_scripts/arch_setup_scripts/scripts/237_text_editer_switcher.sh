#!/usr/bin/env bash
# =============================================================================
# ELITE HYPRLAND TEXT EDITOR SWITCHER - PLATINUM EDITION (v6.2)
# =============================================================================
#
...
# Type 0 = GUI (exec, $textEditor)
# Type 1 = Terminal (exec, $terminal --class $key -e $textEditor)
...
declare -r STATE_FILE="${HOME}/.config/dawn/settings/texteditor_switch"
...
switch_text_editor() {
	...
	local exec_cmd
	if [[ "$t_type" == "1" ]]; then
		exec_cmd="\$terminal --class $target -e \$textEditor"
	else
		exec_cmd="\$textEditor"
	fi
...
draw_ui() {
	...
	# Sub-header
	local curr_txt="Current: ${CURRENT_EDITOR_KEY}"
	strip_ansi "$curr_txt"
	local -i c_len=${#REPLY}
	left_pad=$(((BOX_INNER_WIDTH - c_len) / 2))
	right_pad=$((BOX_INNER_WIDTH - c_len - left_pad))
	printf -v pad_buf '%*s' "$left_pad" ''
	buf+="${C_MAGENTA}│${pad_buf}${C_GREY}Current: ${C_GREEN}${CURRENT_EDITOR_KEY}${C_MAGENTA}"
	printf -v pad_buf '%*s' "$right_pad" ''
	buf+="${pad_buf}│${C_RESET}${CLR_EOL}"$'\n'
...
run_tui() {
	if [[ ! -t 0 ]]; then
		log_err "TUI requires a terminal."
		exit 1
	fi
	...
	if [[ $# -eq 0 ]]; then
		...
		--apply-state)
			if [[ -f "${STATE_FILE}.smart" ]]; then
				...
				# Legacy state handling: 'true' -> nvim, 'false' -> gnome-text-editor
