...
# ==============================================================================
# USER SERVICE ENABLER (Systemd/OpenRC)
# ==============================================================================
# Description: Enables USER services safely and sequentially.
#              Supports both systemd and OpenRC.
# Logic:       Checks NON-Root -> Detect Init -> Iterate Array -> Enable
...
readonly TARGET_USER_SERVICES=(
	"hypridle.service" # Idle daemon (systemd)
	"hypridle"         # Idle daemon (OpenRC)
	"waybar"           # Waybar (OpenRC)
...
main() {
	...
	printf "\n${C_BOLD}Starting User Service Initialization ($init_system)...${C_RESET}\n"
	printf "${C_BOLD}-------------------------------------------------------${C_RESET}\n"

	local svc_name

	for svc_name in "${TARGET_USER_SERVICES[@]}"; do
		# Remove .service suffix for OpenRC
		...
		openrc)
			if rc-service -l 2>/dev/null | grep -q "^${svc}$" || [[ -x "/etc/init.d/$svc" ]]; then
				if rc-update add "$svc" default 2>/dev/null; then
					rc-service "$svc" start 2>/dev/null || true
					log_success "Enabled & Started: ${C_PURPLE}$svc${C_RESET}"
				else
					log_err "Could not enable $svc"
				fi
			else
				log_warn "Service not found: ${C_PURPLE}$svc${C_RESET}. Skipping..."
			fi
			;;
		esac
	done
