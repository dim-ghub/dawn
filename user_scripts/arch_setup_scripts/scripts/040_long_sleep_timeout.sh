...
main() {
	...
	if pgrep -x hypridle >/dev/null; then
		info "Restarting hypridle process..."
		pkill -x hypridle
		sleep 0.5
		hypridle &>/dev/null && success "Hypridle restarted." || warn "Failed to restart hypridle."
	else
		info "Starting hypridle process..."
		hypridle &>/dev/null && success "Hypridle started." || warn "Failed to start hypridle."
	fi
}
